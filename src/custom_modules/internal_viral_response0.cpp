...
//rwh	double R = pCell->custom_data[nR];
	double Rval = pCell->custom_data[nR];
	...
	{
        std::cout << "--------- " << __FUNCTION__ << ": calling pyroptosis_cascade!! (and return)\n";
	...
	if( Rval > 3 && pCell->custom_data["cell_pyroptosis_flag"]==0 && pCell->custom_data["cell_virus_induced_apoptosis_flag"]==0)
	{
	...
	if( Rval  >= 1.0 - 1e-16 ) // if there is at least 1 complete set of uncoated viral RNA
	{
		double scaled_RNA = Rval  / ( pCell->custom_data["interferon_viral_RNA_threshold"] + 1e-32 );
		if( scaled_RNA > 1 )
		{ scaled_RNA = 1.0; }
